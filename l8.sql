...
-- --tinh luong ngay hien tai cua nhan vien tiep an, 60k/h, la ca 2 cua ngay 

-- SELECT MaNV, Ngay, SUM(DATEDIFF(HOUR, GioDen, GioDi) * 60000) as [Luong]
-- FROM PhanCa
...
-- SELECT MaNV, SUM(DATEDIFF(HOUR, GioDen, GioDi)) as [SoGio] FROM PhanCa WHERE Ngay = '2024-5-4'
-- GROUP BY MaNV
...
(
    ...
    CONSTRAINT PK_ThuongPhat PRIMARY KEY (MaNV, Ngay)
)
...
-- them cot ghi chu cho phan ca
ALTER TABLE PhanCa ADD GhiChu nvarchar(255) NULL DEFAULT ''


--thêm cột ngào tạo vào phiếu đăng kí
ALTER TABLE PhieuDangKy ADD NgayTao date NULL DEFAULT GETDATE()

--them gia tri mac dinh cho tinh trang phong
